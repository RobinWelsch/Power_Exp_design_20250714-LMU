...
for _ in range(n_sims):
    a = generate_ordinal(0)
    b = generate_ordinal(shift_AB)
    c = generate_ordinal(shift_C)
    _, p_w = mannwhitneyu(a, b, alternative='two-sided')
    pvals_wilcox.append(p_w)
    _, p_k = kruskal(a, b, c)
    pvals_kruskal.append(p_k)
...
for n_i in ns_vec:
    p_w  = np.empty(n_sims)
    p_kw = np.empty(n_sims)
    
    for i in range(n_sims):
        a = generate_ordinal(0,       n_i)
        b = generate_ordinal(shift_AB, n_i)
        c = generate_ordinal(shift_C,  n_i)
        
        # Wilcoxon (Mann–Whitney U) A vs B
        _, p_w[i] = mannwhitneyu(a, b, alternative='two-sided')
        
        # Kruskal–Wallis A, B, C
        _, p_kw[i] = kruskal(a, b, c)
    
    records.append({
        'n':      n_i,
        'method': 'Wilcoxon',
        'power':  np.mean(p_w < alpha)
    })
    records.append({
        'n':      n_i,
        'method': 'Kruskal–Wallis',
        'power':  np.mean(p_kw < alpha)
    })

power_nonpar = pd.DataFrame.from_records(records)

# 7d. Plot the power curves
plt.figure(figsize=(8, 5))
sns.lineplot(data=power_nonpar, x='n', y='power', hue='method', style='method', markers=True)
plt.axhline(0.8, linestyle='--')
plt.title(f'Non-parametric Power vs. Sample Size (α = {alpha})')
plt.xlabel('Sample Size per Group (n)')
